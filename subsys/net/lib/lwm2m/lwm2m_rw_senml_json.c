...
struct json_in_formatter_data {
	...
	uint16_t value_len; //Inline length
...
static int json_next_token(struct lwm2m_input_context *in, struct json_in_formatter_data *fd)
{
	...
	cont = 1U;
	if (in->block_ctx && in->block_ctx->json_flags) {
		//Store from last sequence
		fd->json_flags = in->block_ctx->json_flags;
		in->block_ctx->json_flags = 0;
	}
...
static size_t put_begin_r(struct lwm2m_output_context *out, struct lwm2m_obj_path *path)
{
...
static size_t put_end_r(struct lwm2m_output_context *out, struct lwm2m_obj_path *path)
{
	struct json_out_formatter_data *fd;
...
static size_t put_float32fix(struct lwm2m_output_context *out, struct lwm2m_obj_path *path,
			     float32_value_t *value)
{
	...
	if (plain_text_put_float32fix(out, path, value) == 0) {
		goto clean_buf;
	}
...
static int write_opaque_buffer(struct lwm2m_output_context *out, char *buf, size_t buflen)
{
	...
	if (base64_encode(CPKT_BUF_PTR(out->out_cpkt), CPKT_BUF_SIZE(out->out_cpkt), &temp_length,
			  buf, buflen)) {
		/* No space avaiable for base64 data*/
		return -1;
	}
...
static size_t get_float32fix(struct lwm2m_input_context *in, float32_value_t *value)
{
	struct json_in_formatter_data *fd;
			 ...
			 struct lwm2m_opaque_context *opaque, bool *last_block)
{
	...
	if (opaque->remaining == 0) {
		...
		if (in->block_ctx) {
			if (in->block_ctx->base64_buf_len) {
				uint8_t module_buf[4];
				size_t buffer_module_length = 4 - in->block_ctx->base64_buf_len;
				if (fd->value_len < buffer_module_length) {
					return 0;
				}

				fd->value_len -= buffer_module_length;
				memcpy(module_buf, in->block_ctx->base64_mod_buf,
				       in->block_ctx->base64_buf_len);
				memcpy(module_buf + in->block_ctx->base64_buf_len, data_ptr,
				...
				in->block_ctx->base64_buf_len = 0;
				if (!in->block_ctx->last_block) {
					in->block_ctx->base64_buf_len = (fd->value_len % 4);
					if (fd->value_len < in->block_ctx->base64_buf_len) {
						return 0;
					}
					...
					if (in->block_ctx->base64_buf_len) {
						uint8_t *data_tail_ptr =
							data_ptr + (original_size -
								    in->block_ctx->base64_buf_len);
						memcpy(in->block_ctx->base64_mod_buf, data_tail_ptr,
						       in->block_ctx->base64_buf_len);
						fd->value_len -= in->block_ctx->base64_buf_len;
					}
				}
				//Decode rest of data and do memove
				...
				if (in->block_ctx->base64_buf_len) {
					uint8_t *data_tail_ptr =
						data_ptr +
						(original_size - in->block_ctx->base64_buf_len);
							 ...
							 uint8_t path_list_size)
{
	...
	if (fd->base_name.level == 0 || path_list_size == 1) {
		return;
	}
	...
	for (int i = 1; i < path_list_size; i++) {
		if (path_list[i].level == 0 || fd->base_name.obj_id != path_list[i].obj_id) {
			...
			fd->base_name.level = 0; //Define just "/" base name
			return;
		}
		...
		if (fd->base_name.level >= path_list[i].level &&
			...
			fd->base_name.level = 1; //Define just "/obj_id/" base name
			continue;
		}
...
int do_composite_read_op_senml_json(struct lwm2m_message *msg)
{
	...
	uint8_t path_list_size = 0;

	path_list_size = json_parse_composite_read_paths(msg, path_list, CONFIG_LWM2M_COMPOSITE_PATH_LIST_SIZE);
	if (path_list_size == 0) {
...
int do_read_op_senml_json(struct lwm2m_message *msg)
{
	...
	/* Init message here ready for response */
	path = msg->path;
	ret = lwm2m_perform_composite_read_op(msg, LWM2M_FORMAT_APP_SEML_JSON, &path, 1);
	engine_clear_out_user_data(&msg->out);
...
int do_write_op_senml_json(struct lwm2m_message *msg)
{
	...
	struct lwm2m_block_context *block_ctx = msg->in.block_ctx;
	...
	if (block_ctx && block_ctx->json_flags) {
		...
		if (block_ctx->base_name_stored) {
			int base_name_length = 0;
			if (block_ctx->base_name_path.level == 0) {
				base_name_length = snprintk(base_name, sizeof(base_name), "/");
			} else if (block_ctx->base_name_path.level == 1U) {
				base_name_length = snprintk(base_name, sizeof(base_name), "/%u/",
							    block_ctx->base_name_path.obj_id);
			} else if (block_ctx->base_name_path.level == 2U) {
				base_name_length = snprintk(base_name, sizeof(base_name), "/%u/%u/",
							    block_ctx->base_name_path.obj_id,
							    block_ctx->base_name_path.obj_inst_id);
			} else if (block_ctx->base_name_path.level == 3U) {
				base_name_length =
					snprintk(base_name, sizeof(base_name), "/%u/%u/%u",
						 block_ctx->base_name_path.obj_id,
						 block_ctx->base_name_path.obj_inst_id,
						 block_ctx->base_name_path.res_id);
			} else {
				base_name_length =
					snprintk(base_name, sizeof(base_name), "/%u/%u/%u/%u",
						 block_ctx->base_name_path.obj_id,
						 block_ctx->base_name_path.obj_inst_id,
						 block_ctx->base_name_path.res_id,
						 block_ctx->base_name_path.res_inst_id);
			}
		...
		if (block_ctx->full_name_true) {
			//base_name_path
			int full_name_length = 0;
			if (block_ctx->resource_path_level == 3U) {
				full_name_length =
					snprintk(full_name, sizeof(full_name), "/%u/%u/%u",
						 block_ctx->base_name_path.obj_id,
						 block_ctx->base_name_path.obj_inst_id,
						 block_ctx->base_name_path.res_id);
			} else {
				full_name_length =
					snprintk(full_name, sizeof(full_name), "/%u/%u/%u/%u,",
						 block_ctx->base_name_path.obj_id,
						 block_ctx->base_name_path.obj_inst_id,
						 block_ctx->base_name_path.res_id,
						 block_ctx->base_name_path.res_inst_id);
	...
	while (json_next_token(&msg->in, &fd)) {
		...
		case SENML_JSON_NAME_ATTRIBUTE:
			...
			if (block_ctx) {
				//Store Resource data Path to base name path but store separately path level
		...
		case SENML_JSON_STRING_BLOCK_DATA:
			//Opaque stream end detetcted
			data_value = true;
			break;

		case SENML_JSON_UNKNOWN_ATTRIBUTE:
			LOG_ERR("Unknow attribute");
			ret = -EINVAL;
			goto end_of_operation;
			break;
		default:
